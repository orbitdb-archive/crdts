 ...
 */
export default class CmRDTSet<V=unknown, T=unknown> extends Set<V> {
	// Internal cache for tracking which values have been added to the set
	protected _values = new Set<V>()
  ...
  values (): IterableIterator<V> {
    const shouldIncludeValue = (e: OperationTuple3<V, T>) => this._resolveValueState(e.added, e.removed, this._options.compareFunc)
    const getValue = (e: OperationTuple3<V, T>) => e.value
   ...
   */
  hasAll (values: V[]): boolean {
    const contains = (e: V) => this.has(e)
  ...
  add (value: V, tag: T): this {
    ...
    if (!this._values.has(value)) {
      // Create an operation for the value and apply it to this set
      const addOperation = OperationTuple3.create<V, T>(value, [tag])
      this._applyOperation(addOperation)
    } else {
      // If the value is in the set, add a tag to its added set
      this._findOperationsFor(value).map(val => val.added.add(tag))
   ...
   */
  toJSON (): { values: { value: V, added: T[], removed: T[] }[] } {
   ...
   */
  isEqual (other: CmRDTSet<V, T>): boolean {
   ...
   */
  protected _applyOperation (operationTuple3: OperationTuple3<V, T>) {
   ...
   */
  protected _findOperationsFor (value: V) {
    let operations: OperationTuple3<V, T>[] = []
    if (this._values.has(value)) {
      const isForValue = (e: OperationTuple3<V, T>) => e.value === value
      const notNull = (e?: OperationTuple3<V, T>) => e !== undefined

      operations = [this._operations.find(isForValue)].filter(notNull) as OperationTuple3<V, T>[]
    }
    return operations
  }
   ...
   */
  static isEqual<V=unknown, T=unknown> (a: CmRDTSet<V, T>, b: CmRDTSet<V, T>) {
   ...
   */
  static difference<V=unknown, T=unknown> (a: CmRDTSet<V, T>, b: CmRDTSet<V, T>) {
    const otherDoesntInclude = (x: V) => !b.has(x)
